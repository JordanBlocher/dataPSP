...
\subsubsection{The winning bid is persistent private information.}
...
\proof{Follows directly from the result of Proposision \ref{userunion}.}
...
accurate correspondence\footnote{This is known as the Correspondence theory of
truth}. Using a set-theoretical approach to address the sellers
...
buyer-seller pair. Let $S = \Pi_{j\in\mcI} \big(
\Pi_{i\in\mcI} S_i^j \big)$. The indicator function is the canonical mapping,
...
$P = \big\lbrace [s] : s\in S\big\rbrace$ of $S$. } Therefore each seller
$j\in\mcI$ is equivalent buyer(s) $i\in\mcI$. That is, buyers and
sellers with an equality relation (\ref{eqrel}) belong to the same local
auction equilbrium. Therefore, sellers' bids represent $\epsilon$-best replies
in the strategy space. $\qed$ (DONE? REALLY?)
...
{
\lemma{(Local Static Nash Equilibrium)} 
...
justified, and we have a local equlibirium at time $\tau$.
$\qed$}
