...
%\epsilon I!)$.
%We note that the convergence time is 
...
Progressive second price auctions are used for optimal allocation in a
...
Formally,
\definition{(Anonymous auction)} \cite{felix}
...
\subsubsection{The winning bid is trusted (anonymous) information.}
...
%purchase. We do not model ISP revenue, but assume it may be extracted from the bid fee at $t=0$.
...
% MECHANISM
\subsubsection{The Mechanism}\label{datamechanism}
...
%bandwidth given in \cite{lazar}. 
%We note that if a single seller $j$ can
...
In any market, a buyer or seller would like to obtain the maximum amount of utility 
...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
  {
     ...
     d &  \\};
  \path[-stealth]
    (m-1-1) edge node [left] {projection} (m-2-1)
            edge node [above] {level} (m-1-2)
    (m-1-2) edge [dashed,-] (m-2-1);
\end{tikzpicture}
...
marketplace. 
Naturally, a buyers' valuation is aggregated over local markets, and the
sellers' valuation is aggregated over its own auction. We have already introduced the composition $\theta_i
...
derivatives
${\theta_i}$ and ${\theta^j}'$ satisfying the conditions of elastic demand.
}
\proof{
Let $\xi$
...
Assuming equal bandwidth for all users, we derive a globally optimal strategy suited for
...
Consider a user purchasing data from a subset of other network users.
...
following Corollary, 
{
\corollary{($\Delta$-Pareto efficiency)}
...
\proof{Follows directly from the result of Proposition \ref{userunion}.}
...
Thus, we construct an extension %to be all-inclusive, The game-theoretical basis of our argument is complemented by an abstract-theoretical analysis.
...
% Sorry, didn't check this one, wow need it to work, ref https://math.stackexchange.com/questions/2264090/in-brouwers-theorem-how-does-the-absence-of-fixed-points-imply-certain-retract
\proof{
For a seller-buyer pair, which we will denote $(i,j)$, let $y=\theta_i'(\g_*^j(a))$.
We use the the axiom of set equality based on first-order logic,
which states that, $\forall \ i\in\mcI, \ \forall \ j\in\mcI, (i\in \mcI^j \Leftrightarrow j\in\mcI_i) \Rightarrow i \thicksim j$, and follows from (\ref{jtoi}).  Then, for any allocation $a$, we may define the relation, $i\thicksim j$ as,
 % dual?
\begin{equation}\label{eqrel}
        d_i^*(y) = \g_*^j(a) \qquad \text{AND} \qquad f^j \circ \g_i^*(a) = y,
\end{equation}
 where $f$ is defined as in (\ref{sellerinversedemand}).
\vspace{-0.3in}
{
\begin{center}\begin{figure}[H]\caption{Buyer-seller pair}
{
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,row sep=3em,column sep=4em,minimum width=2em]
  {
...
\vspace{-0.3in}
We have that $f_i=f^j$, from (\ref{uservaluation}), i.e., for any $z\in D_i$, $x\in D^j$, 
\begin{equation}\label{zerodiff}
    \big[\theta_i(x) -\theta_i(x-\xi)\big] \quad \text{AND} \quad \big[\theta^j(z) - \theta^j(z-\zeta)\big].
\end{equation}
So, 
$$
    \big(\theta_i(x)\theta^j(z) - \theta_i(\xi)\theta^j(\zeta)\big)^2 = 4\theta_i(x)\theta^j(z).
$$
Since from (\ref{zerodiff}), $\theta_i\theta^j = $
Formally, the set of users in an auction is uniquely determined by its members. We note that $\thicksim$ defines equality of bids using a \emph{static}
analysis, meaning that sellers have fixed market prices; all users who are not changing their bids are considered equal.
Define the mapping $s \mapsto t$ such that, for any $z$,
\begin{equation}\label{indeqrel}
         \theta_i'(z) - {\theta^j}'(z) \le \epsilon/2.
\end{equation}
and so,
\begin{equation}\label{indeqrel}
...
$$
Now, the set of all equivalence classes (DEFINE % need https://math.stackexchange.com/questions/2264090/in-brouwers-theorem-how-does-the-absence-of-fixed-points-imply-certain-retract
...
$\mcI_i$, i.e. $\g_i \circ B_i^\epsilon \equiv B_i^\epsilon\vert_{\mcI_i}$ . % Why did I use size here? ... don't remember
Thus, we have $\g_i \circ T_i = ([0,D^k]_{k\in\mcI^j}\times [0,\g_i \circ
...
Tychonoff's theorem, i.e. every product of a compact space is compact, we have $\g_i \circ B_i^\epsilon$
...
As a result of user behavior, and subsequent strategies, we determine that the data-exchange market behaves in a predictable way. However,
...
\proof{
As auction $j$ is at equilibrium, and 
since $\theta_i'$ is continuous, as was shown in Lemma \ref{uservaluation}, and
$t=[t_i^j] \in \lambda^j \times \Lambda^j$ is continuous in $s$ on
$T_k=\Pi_{k\in\lambda^j} T_k^j$. Now, $t$
represents a continuous mapping of $[0,\sum_{k\in\lambda^j} D^k]_{i\in\Lambda^j}$ onto itself and we may use Brouwer's fixed point theorem, as in \cite{semret}, which states that the continuous mapping of a convex compact set into itself has at least one fixed point. Therefore, $\exists \ z^* = \sigma^*(z) \in [0,D_i]_{i\in\Lambda^j}$. So, given that $s^* = (z^*, \theta'(z^*))$, we have that $s^* = t(s^*)\in T$.
%We have that $\forall s \in T$, $\forall z >0$, we have $z>0 = \sum_{i\in \Lambda^j} D_i(p_{i}^j/2,s_{-i})$, so using ($\ref{dataprice}$), $ \sum_{i\in\lambda^j}P_{i^*}^j(z,s_{-i}) \ge p_*^j$ and truthful bid $t_i = (v_i, \theta'(z)) \forall i \in \lambda^j$
$\qed$
}\\

We have proven that buyers will submit bids according to their marginal valuations. We have that all bids represent $\epsilon$-best replies, and, as was proven in
...
%in an analogy to the property of the same name for functions?
...
$$
is well-posed by Hadamard [1923], definition (4.3) and Corollary (4.4) (PROVE?). %isomorphic?
...
{
\corollary{Hemicontinuity of $\Delta^\epsilon$}\\
...
\proof{We have that for any $i$, $\forall \ y_i \ne \theta'(z*)$
$$
    u_*^j \ge u^j(z^*,y_i) - \epsilon/2 - \varrho,
$$
and so we have that $[s] \mapsto t \in T^{\epsilon}$ is defined by $\Gamma: \Lambda \mapsto \lambda$ $\forall \ i \in \lambda$. This is the coassociative, cocommunicative property, where for some pair $(i,j) \in \Delta, u_*^j = u_i^* = u_i^j$. 
The amount of data offered by a seller $j$ at price $p^*$ is met by a buyer $i$ at price $p_*$; we have that there must be some buyer-seller pair $(i,j)$ at some $t\in\tau_i\times \tau^j$ such that $p^* \ge p_*^j \ge p_i^* \ge p_*^$, and $u_* \simeq u^*$. Our cover is formed by the duality of \ref{buyervaluation} and \ref{sellervaluation}, and so we have hemicontinuity by the relation $R \equiv \theta_i \propto \theta^j$ for any $(i,j)\in\Delta^\epsilon$, and we assume the existence of a non-transient, global state, which may or may not be the termination of all auctions.\\ % which several non-degenerate maps may be chosen from to satisfy all constraints
\textbf{Remark:} We note that $\tau_i$ does not need to equal $\tau^j$ in order for equivalence to hold, as at some point there will be at least one pair, $(i,j)$, which satisfy the constraints. In the simplest case, there are N buyers and N sellers, and if a stable matching exists, then (\ref{co1}) $\iff$ (\ref{co2}).}\\

We have shown that our bidding strategy results in at least one \emph{local} Nash equilibrium. Now, we must prove that each bidding process converges to a unique auction, i.e., no buyer is assigned data that was not bid on, and no seller receives any false bids, which we have by assumption. We first show the existence of a \emph{static} Nash equilibrium, where the sellers reserve prices are fixed.
{
\corollary{(Primary fixed point)}
Let $\Lambda, \lambda$ be defined as in \ref{userunion}. If
${\Lambda}$ is not a partition, i.e. $\nexists \ j,k
\in{\Lambda}$ such that $\Lambda^j \cap \Lambda^k = \emptyset$, then,
for fixed $t$,
$\exists \ j \in{\Lambda}$ such that $p_*^j \ge p_*^k, \ \forall \
k\ne j\in{\Lambda}$. 
}\\
...
above and below. From the assumption that ${\Lambda}$ is not a
partition, we have that the limits exist with respect to bid price $p_i^j$, 
$$
    \limsup\limits_{j\to I} {\Lambda} =  \bigcap_{j\ge 1}\bigcup_{k\ge j} \Lambda^k,
$$
is the primary seller $j$, and we have the market price $p_*^j$ from,
$$
    \liminf\limits_{i\to I} {\lambda} = \bigcup_{i\ge 1}\bigcap{k\ge i}
\lambda^k,
$$
...
Finally, we note the harmonious relationship of users, data, and mathematics.
The exchange of data in the secondary market must mimic the topology of the
strategy space. Thus, although we assume that the message space of PSP auctions
is too small, we conjecture that this relationship is the backbone of an
evolutionary system, naturally derived from the continuity of demand and
shifting market equilibria.
...
$p^\kappa$ represents the price for data set by the ISP, which we note is also
the upper bound of the sellers' pricing function. We note that in \cite{zheng}
this cost is the data overage fee.
...
$p^\kappa$ is reached. Thus, as in each case we have that $i$ is able to satisfy their demand,
and we determine that the opt-out strategy is optimal.
$\qed$}


\subsection{Proof of Lemma 3.6}
\proof{
...
aggregate demand of subset $\mcI^j$ is satisfied by seller $j$.
...
$j$ and buyer $i$ is well-posed, the reserve price (\ref{newprice}) is
justified, and we have a local equilibrium at time $\tau$.
$\qed$}
...
that $\forall \ j\in\mcI_i$, $y_i^j \ge p_{i^*}^j$, where $p_{i^*}^j$ is the reserve price of seller $j$, defined in Proposition
...
$\qed$}
%Finally, we note that $z_i^j$ does not increase the aggregate demand of the buyer.
