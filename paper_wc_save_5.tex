...
% OPT-OUT BUYER
We define a \textbf{opt-out buyer} as a user $i\in\mcI$ with quasi-linear utility
function, the value of the allocation minus the cost,
\begin{equation}\label{buyerutility}
    u_i = \theta_i \circ e - c_i,
\end{equation}
where $e : [0, \infty)_{i\in\mcI} \rightarrow [0,\infty)_{j\in\mcI}$ is %the expectation 
...
which $i$ \emph{could} buy the unit from \emph{any} seller $j\in\mcI$.
...
\begin{equation}\label{sellercoordinate}
    n = \min(i\in\ell^j : \displaystyle\sum_{i\in\mcI} d_i^j \ge
D^j),
\end{equation} 
...
\end{equation}
$\forall i \in \mcI^j$, and update $j$'s total data to reflect the (tentative) allocation,
...
scenario. The seller may simply add back in the allocated data,
$$
...
necessary to formally define (and restrain) the incentives of the users. 
...
$k\ni\mcI_i$,
$$
    D_i(y_i^j) \ge D_i(p_i^{j^*})\ge D_i(p_i^{k}).
$$
...
We proceed to examine the strategy of the users in local auctions. As we have shown, the seller
is a functional extension of the buyer, with rules determined by the buyers'
behavior. Seller $j$'s reserve price is determined by a
...
equilibrium (NOTE: NOT GLOBAL! YET). We have the following Proposition.
...
\begin{align}
\begin{split}
    z_i &= \sup\bigg\lbrace z\ge 0 : 
 {\theta_i}'(z) > \bar{P}_i^j(z)\bigg\rbrace, 
\end{split}\\
\begin{split}
    \chi_i &= \sup\bigg\lbrace z\ge 0: 
\displaystyle\int_0^z 
    \bar{P}_i(z) \ dz \le \sum_{j\in\mcI_i} b_i^j \bigg\rbrace,
\end{split}
\end{align}
...
We have a ``truthful" local game embeddeded in each local auction, and an
equilibrium point of
the embedded local game is an equlibrium point for the local auction. 
}\\
\textbf{Proof:}
For the buyer, we show that $t_i$ is and $\epsilon$-best reply. That is,
$$
    u_i(t_i;s_{-i}) + \epsilon \ge u_i(s_i;s_{-i}).
$$
We have that $p_i^j=p_{i^*}^j \ \forall i\in\mcI^j$ and is the lowest price that $j$
will accept to perform any allocation. By the definition of ``reserve
price", $j$ is willing to buy (all) its own data $D^j$ at price $p^j$. For
each $i \in \mcI^j$, 
$$
    y_i^j = \min_{j\in\mcI_i} (p_i^j).
$$
Let $e = z_i^j$, since $\theta_i'$ is non-increasing
and, $P_i^j$ is non-decreasing, and so 
$$
    \theta_i'(e) = w_i^j  \ge P_i^j(v_i^j),
$$ 
holds for any $j$. Therefore, as $i$'s strategy guarantees that $i\in\mcI^j$, 
and (\ref{buyerinversedemand}) defines $e$ as being
max of the reserve price $p^j$ of any $j\in\mcI_i$, 
\begin{align*}
    v_i^j \le D^j - d^j(w_i^l).
\end{align*}
