...
\newtheorem{corFollary}{Corollary}[section]
...
decrease the potential cost of its data, in exchange for additional messaging overhead, possible
fees, and increased competition from other buyers. This aspect of
a user transaction represents potential \emph{indirect} costs, which may be considered work done to find
sellers, monetary fees, or effort of communication from participation. A seller
has the potential for greater profit with each new buyer in its auction, with
the same liabilites. We
define this cost to be separate from the bid fee $\epsilon$, as it is dependent on the underlying network
and the individual. We denote the potential indirect costs of participation of
each user as $\varrho$. Our mechanism allows a buyer
...
\begin{equation}\label{buyerutility}
    u_i = \theta_i \circ (\g_i(a)) - \g_i(\varrho) - c_i,
\end{equation}
...
of the resources and their decision strategies. Also, we reason that buyers,
...
\begin{split}\label{buyercoordinate}
    \min\big\lbrace n \in \mcI \ &\vert \ n D^n\ge D_i \\
    %\wedge \sum_{j\in\mcI(n)}&\g_i^j(\varrho) \le \sum_{j\in\mcI(n)} \theta_i^j(\g_i^j(a)) \big\rbrace. 
    &\wedge \g_i(\varrho) \le \theta_i(\g_i(a)) \big\rbrace. 
\end{split}
\end{align} 
...
the demand is satisfied, the debit from the overhead costs are balanced, or the upper bound of the sellers' reserve price
...
exist.
{
\sublemma{Seller cooperation.}
We claim that, in addition, $s^j$ is both truthful and optimal.
For a single buyer-seller interaction, (\ref{jtoi}) gives that $\theta_i =
\theta^j \Rightarrow \theta_i' = {\theta^j}'$. Thus, by symmetry, and as by
(\ref{newprice}), $j\in\Lambda$ cannot set a higher reserve price, 
...
Finally, as we may consider the subset $\Lambda \cup \lambda$ to be a
multi-auction game, our user strategies form a ``truthful" local game $\subset
\mcI$ with strategy space restricted to $\epsilon$-best replies from buyers $\in
\lambda$. Therefore we have that a fixed point in the ``truthful" game is a fixed point for the auction. 

We determine that the data-sharing market consists of a union of these multi-auction games around fixed points. Clearly, the union of all possible
sets $\Delta = \Lambda \cup \lambda$ covers $\mcI$. Consider two subsets, $A$
and $B$, at equilibrium, and a shared
buyer $k\in \lambda_A \cup \lambda_B$.  By Lemma
\ref{userunion}, a transition is induced in $B$ by $A$ if and only if
$\exists \ j \in \Lambda_A$ such that $p_*^{j}\ge p_*^{k}$, for some
$k\in\Lambda_B$. We claim that the shared buyers
between the different subsets form a heirarchical structure over the subsets
$\Delta$. 
...
(EXPAND)
We first have the static Nash equilibrium, where buyers have fixed reserve
prices.
