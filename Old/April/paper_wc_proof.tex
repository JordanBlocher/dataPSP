...
therefore we identify each local auction
...
we reason that there remains an optimal user strategy suited for the
data-sharing model. As a user no longer needs
...
***********************************
\fi
Fixing all bids $s_i^j \in \mcI$, and time $t\in\tau$, (ONLY HAVE ONE HALF OF THE SYMMETRY SO FAR! REALLY WANT TO MAKE $s_i = s^j$
HAPPEN)
}\\
\textbf{Proof:}
...
Furthermore, using (\ref{datademand}), it follows that,
\begin{equation}\label{jdemand}
    \displaystyle\sum_{i\in\mcI^j}\int_0^{e_i^j(a)} f_i(z) \ dz = \int_0^{D^j}
f^j(z) \ dz.
\end{equation}
...
Now, suppose $\exists \ l\in\mcI^k$ such that $l\in\mcI^j \Rightarrow p_l^* \ge
...
for $q\in\mcI_l$ or $\theta_l^q = 0$ for $q\in\mcI_i$. Thus, any bid ouside of
our construction
has a zero valuation, with respect to buyers $\in\lambda$ and sellers
$\in\Lambda$. We have,
\begin{equation}\label{idemand}
    \displaystyle\int_0^{D_i} f_i(z) \ dz = \sum_{k\in\mcI_i}\int_0^{e_i^k(a)}
f_i(z) \ dz,
\end{equation}
\begin{align*}
  \int_0^{D^j} f^j(z) \ dz = \displaystyle\sum_{i\in\mcI^j}\int_0^{e_i^j(a)} f^j(z) \ dz = \int_0^{e_l^k(a)} f^k(z) \ dz.
\end{align*}
Thus, by construction, 
\iffalse
\begin{align*}
    \displaystyle\sum_{i\in\mcI^j} &\bigg( \sum_{k\in\mcI_i} \int_0^{e_i^k(a)}
f^k(z) \ dz \bigg) \\
    &= \sum_{i\in\mcI^j}\bigg(\sum_{k\in\mcI_i}\bigg(\sum_{l\in\mcI^k}\int_0^{e_l^k(a)} f^k(z) \ dz.
\bigg)\bigg)
\end{align*}
\fi
the continuity of $\theta$, and using (\ref{jdemand}), (\ref{idemand}),
\begin{align*}
...
Finally, we have,
\begin{equation}
    \displaystyle\sum_{j\in\Lambda} \int_0^{e_i^j(a)} f_i(z) \ dz = \sum_{i\in\lambda}\int_0^{D^j} f^j(z) \ dz.
\end{equation}
